...
public class IccInstrumentDestination 
{
        	...
        	for (SootMethod sm : sootMethods)
        	{
        		if (sm.getName().equals("<init>"))
        		{
        			if (sm.getParameterCount() == 1 && sm.getParameterType(0).equals(INTENT_TYPE))
        			{
        				...
        				superU = (Unit) Jimple
                  .v().newInvokeStmt(Jimple.v().newSpecialInvokeExpr(thisLocal, sm.makeRef(), args));
        				continue;
        			}
     ...
     * @return
     */
    public SootMethod generateGetIntentForActivityResultMethod(
      SootClass compSootClass, SootField intentSootField)
    {
					...
					*/
	    	 		...
	    	 		*/
	    	 		Unit setIntentU = Jimple.v().newAssignStmt(
	    					intentV,
	    					Jimple
                  .v().newVirtualInvokeExpr(thisLocal, hostComponent.getMethodByName("getIntent").makeRef()));
    ...
    public void fieldSendingMessage(SootMethod fromSM)
    {
		...
		int m = Modifier.PUBLIC | Modifier.STATIC;
		SootField sf = Scene
      .v().makeSootField("message_for_ipc_static", RefType.v("android.os.Message"), m);
		fromC.addField(sf);
		    	...
		    	for (Iterator<Unit> iter = units.snapshotIterator(); iter.hasNext(); )
		    	{
		    		...
		    		if (stmt.containsInvokeExpr())
		    		{
		    					...
		    					equalsOrSubclassOf(toBeCheckedMethod.getDeclaringClass(), Scene.v().getSootClass("android.os.Messenger")))
		    			{
		    				...
		    				Unit assignUnit = Jimple
                  .v().newAssignStmt(Jimple.v().newStaticFieldRef(sf.makeRef()), arg0);
